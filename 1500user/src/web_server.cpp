...
static void deleteRecord() {
  ...
  if (db_exec(test1_db, sql.c_str()) == SQLITE_OK) {
    web_content += "OK";
    Serial.println(web_content);
    // Chỉ xóa template trên sensor nếu fpid trong range cảm biến (1..FP_MAX_ID).
    // Seed data fpid > FP_MAX_ID không có template thật → skip để khỏi log lỗi nhầm.
    if (dfpid >= 1 && dfpid <= FP_MAX_ID) {
      uint8_t fpResult = deleteFingerprint((uint16_t)dfpid);
      ...
      Serial.print("[DeleteRecord] fpid out of sensor range (>");
      Serial.print(FP_MAX_ID); Serial.print("), skip sensor delete: ");
...
static void syncCheck() {
  ...
  // DB: lấy fpid trong range sensor (1..FP_MAX_ID)
  syncDbCsv_ = "";
  char *zErr = NULL;
  String dbSql = "SELECT fpid FROM attendance WHERE fpid >= 1 AND fpid <= ";
